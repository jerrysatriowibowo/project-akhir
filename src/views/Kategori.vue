<template>
    <div class="bg-gray-800 grid grid-cols-4 gap-4">
      <div v-for="kategori in getKategori.data.slice(0, 12)" :key="kategori.name">
    ...
    <div class="group border-gray-100/30 flex w-full max-w-xs flex-col self-center overflow-hidden rounded-lg border bg-gray-700 shadow-md">
      <a class="relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl" href="#">
        <img class="peer absolute top-0 right-0 h-full w-full object-cover" :src="kategori.banner" alt="product image" />
      ...
      <div class="mt-4 px-5 pb-5">
        <center>
          <h5 class="text-xl tracking-tight text-white">{{ kategori.name }}</h5>
          <h5 class="text-xl tracking-tight text-white">{{ kategori.slug }}</h5><br>
        </center>
